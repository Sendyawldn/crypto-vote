...
CryptoVote uses typed seed data and can persist admin election state through the official MongoDB Node.js driver when `MONGODB_URI` is configured. If MongoDB is not configured, `/api/admin/election` falls back to in-memory state so the presentation demo still works offline. MongoDB driver setup source: https://www.mongodb.com/docs/drivers/node/current/, fetched 2026-05-13.
